...
New-Service -name heartbeat `
  ...
  -binaryPathName "`"$workdir\heartbeat.exe`" -c `"$workdir\heartbeat.yml`" -path.home `"$workdir`" -path.data `"C:\ProgramData\heartbeat`" -path.logs `"C:\ProgramData\heartbeat\logs`" -E logging.files.redirect_stderr=true"
